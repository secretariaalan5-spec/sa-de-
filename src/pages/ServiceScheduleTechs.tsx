...
export default function ServiceScheduleTechs() {
    ...
    const { entries, addEntry, removeEntry, getEntriesForDate, calculateStats } = useServiceSchedule('tech');

    const techs = data.professionals.filter(p => {
        const func = data.functions.find(f => f.id === p.functionId);
        return func?.name.toLowerCase().includes('técn') || func?.name.toLowerCase().includes('tecn');
    });
    ...
    const stats = calculateStats(techs);

    return (
        <div className="animate-fade-in">
            <PageHeader
                title="Escala de Serviço - Técnicos"
                description="Gerenciamento de escalas mensais de eventos para técnicos de enfermagem"
            />
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <Dialog>
                                        ...
                                        <DialogContent>
                                            <DialogHeader>
                                                <DialogTitle>
                                                    Adicionar Técnico - {format(day, 'dd/MM/yyyy')}
                                                </DialogTitle>
                                            </DialogHeader>
                                ...
                                <div className="space-y-1">
                                    {dayEntries.map(entry => {
                                        const professional = techs.find(p => p.id === entry.professionalId);
                                        return (
                                            <div
                                                key={entry.id}
