...
import { X, AlertCircle, AlertTriangle, Megaphone, WifiOff, Wifi, Signal } from 'lucide-react';
...
function NetworkBanner() {
  ...
  useEffect(() => {
    if (!isOnline) {
      ...
      // Acabou de reconectar — mostra feedback breve
  ...
  if (!isOnline) {
    return (
      <div className="w-full bg-slate-800 text-white px-4 py-2.5 flex items-center justify-center gap-2.5 text-xs font-semibold z-50 relative">
        <WifiOff size={13} className="shrink-0 text-slate-300" />
        <span>Sem conexão — exibindo dados salvos. Ações de escrita estão bloqueadas.</span>
      </div>
    );
  }
  ...
  if (showReconnected) {
    return (
      <div className="w-full bg-emerald-600 text-white px-4 py-2.5 flex items-center justify-center gap-2.5 text-xs font-semibold z-50 relative animate-in slide-in-from-top-1 duration-300">
        <Wifi size={13} className="shrink-0" />
        <span>Conectado! Sincronizando dados...</span>
      </div>
    );
  }
  ...
  if (isSlowConnection) {
    return (
      <div className="w-full bg-amber-500 text-white px-4 py-2 flex items-center justify-center gap-2.5 text-xs font-semibold z-50 relative">
        <Signal size={13} className="shrink-0" />
        <span>Conexão fraca ({connectionType}) — o sistema pode demorar um pouco.</span>
      </div>
    );
  }
...
// ─── GlobalBanner (avisos administrativos) ────────────────────────────────────
export function GlobalBanner() {
  ...
  useDataSubscription(['notifications'], () => {
    load();
  });
  ...
  // Sempre renderiza o NetworkBanner por cima de qualquer coisa
  const networkBanner = <NetworkBanner />;

  if (banners.length === 0) return networkBanner;
